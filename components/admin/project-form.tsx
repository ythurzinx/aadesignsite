...
export function ProjectForm({ project }: { project?: Project }) {
  ...
  return (
    <form onSubmit={submit}>
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><Link href="/admin/projetos" className="inline-flex items-center gap-2 text-xs font-bold text-[#637086]"><ArrowLeft className="h-4 w-4" />Voltar</Link><h1 className="display mt-4 text-5xl font-black uppercase">{project ? "Editar projeto" : "Novo projeto"}</h1></div><div className="flex gap-2"><button type="submit" onClick={() => set("published", false)} disabled={saving} className="rounded-full border border-[#07152f]/15 px-5 py-3 text-xs font-black uppercase tracking-[0.1em]">Salvar rascunho</button><button type="submit" onClick={() => set("published", true)} disabled={saving} className="button-light">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Publicar</button></div></div>
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[1fr_22rem]">
        <div className="space-y-6">
          ...
          <section className="rounded-2xl bg-white p-5 sm:p-7"><h2 className="font-bold">Mídia principal</h2><div className="mt-6 grid gap-5 sm:grid-cols-2"><label className="rounded-xl border border-dashed border-[#07152f]/20 p-5 text-center text-xs font-bold"><ImagePlus className="mx-auto mb-3 h-5 w-5 text-[#0b66ff]" />Enviar capa<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" className="sr-only" onChange={(e) => e.target.files?.[0] && uploadField(e.target.files[0], "cover_url")} /></label><label className="rounded-xl border border-dashed border-[#07152f]/20 p-5 text-center text-xs font-bold"><Upload className="mx-auto mb-3 h-5 w-5 text-[#0b66ff]" />Enviar vídeo<input type="file" accept="video/mp4,video/webm,video/quicktime" className="sr-only" onChange={(e) => e.target.files?.[0] && uploadField(e.target.files[0], "video_url")} /></label><label className="text-xs font-bold sm:col-span-2">URL ou referência da capa<input className="field mt-2" type="text" value={values.cover_url ?? ""} onChange={(e) => { set("cover_url", e.target.value); setCoverPreview(e.target.value); }} /></label><label className="text-xs font-bold sm:col-span-2">URL ou referência do vídeo<input className="field mt-2" type="text" value={values.video_url ?? ""} onChange={(e) => set("video_url", e.target.value)} /></label></div></section>
